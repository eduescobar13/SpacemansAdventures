...
//-----> DECLARACIÓN DE LOS MÉTODOS DE LA CLASE.
	...
	public void try_node(int i, int j, boolean direction, node father) { // Método para ir comprobando los nodos, recibiendo las coordenadas y si el movimiento es diagonal.
		if(fin == false) { // CONDICIÓN: Si no se ha llegado al final.
			...
			if(dashboard.get_value(i,j) <= 3) { // CONDICIÓN: Si el nodo es un obstacle, lo ignoramos.
				return;
			}
			...
			if(list_open.find(node) == true) { // CONDICIÓN: Si el nodo está en la lista abierta.
				...
				list_node node_actual = list_open.first; // Almacenamos en una variable local el nodo actual en que nos encontramos.
	...
	public list beggining() { // Método que comienza a ejecutar el algoritmo.
		...
		while(fin != true) { // Mientras no se llegue al final, realizamos un pop de la lista abierta y un push de la lista cerrada.
			...
			if((i != 0) && (j != 0) && (i != dashboard.get_rows() - 1) && (j != dashboard.get_columns() - 1)) { // CONDICIÓN: Si los nodos no pertenecen a los bordes.
				...
				else if(j == dashboard.get_columns() - 1) { // CONDICIÓN: Y está en la esquina superior derecha.
					try_node(i,j-1,false,intermediate); // Tratamos el nodo de la izquierda.
					try_node(i+1,j,false,intermediate); // Tratamos el nodo inferior.
					try_node(i+1,j-1,true,intermediate); // Tratamos el nodo de la diagonal inferior izquierda.
				}
			...
			else if(i == dashboard.get_rows() - 1) { // CONDICIÓN: Si el nodo se encuentra en el borde inferior.
				if(j == 0) { // CONDICIÓN: Y está en la esquina inferior izquierda.
					try_node(i,j+1,false,intermediate); // Tratamos el nodo de la derecha.
					try_node(i-1,j,false,intermediate); // Tratamos el nodo superior.
					try_node(i-1,j+1,true,intermediate); // Tratamos el nodo de la diagonal superior derecha.
				}
				else if(j == dashboard.get_columns() - 1) { // CONDICIÓN: Y está en la esquina inferior derecha.
					try_node(i,j-1,false,intermediate); // Tratamos el nodo de la izquierda.
			...
			else if(j == dashboard.get_rows() - 1) { // CONDICIÓN: Si el nodo se encuentra en el borde izquierdo.
				try_node(i,j-1,false,intermediate); // Tratamos el nodo de la izquierda.
				try_node(i+1,j,false,intermediate); // Tratamos el nodo inferior.
				try_node(i-1,j,false,intermediate); // Tratamos el nodo superior.
				try_node(i-1,j-1,true,intermediate); // Tratamos el nodo de la diagonal superior izquierda.
